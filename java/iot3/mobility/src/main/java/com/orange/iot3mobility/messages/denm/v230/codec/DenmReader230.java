...
import com.orange.iot3mobility.messages.denm.v230.validation.DenmValidationException;
...
public final class DenmReader230 {

    private final JsonFactory jsonFactory;

    public DenmReader230(JsonFactory jsonFactory) {
        this.jsonFactory = jsonFactory;
    }

    public DenmEnvelope230 read(InputStream in) throws IOException {
    ...
    private DenmMessage230 readMessage(JsonParser parser) throws IOException {
        expect(parser.getCurrentToken(), JsonToken.START_OBJECT);
    ...
    private AlacarteContainer readAlacarte(JsonParser parser) throws IOException {
        ...
        StationaryVehicle stationaryVehicle = null;
        ...
        while (parser.nextToken() != JsonToken.END_OBJECT) {
            ...
            switch (field) {
                ...
                case "road_works" -> roadWorks = readRoadWorks(parser);
                case "positioning_solution" -> positioningSolution = parser.getIntValue();
                case "stationary_vehicle" -> stationaryVehicle = readStationaryVehicle(parser);
                default -> parser.skipChildren();
            }
        }
    ...
    private RoadWorks readRoadWorks(JsonParser parser) throws IOException {
        expect(parser.getCurrentToken(), JsonToken.START_OBJECT);
        Integer lightBarSirenInUse = null;
        ClosedLanes closedLanes = null;
        List<Integer> restriction = null;
        Integer speedLimit = null;
        CauseCode incidentIndication = null;
        List<DeltaReferencePosition> recommendedPath = null;
        DeltaReferencePosition startingPointSpeedLimit = null;
        Integer trafficFlowRule = null;
        List<ActionId> referenceDenms = null;

        while (parser.nextToken() != JsonToken.END_OBJECT) {
            String field = parser.currentName();
            parser.nextToken();
            switch (field) {
                case "light_bar_siren_in_use" -> lightBarSirenInUse = parser.getIntValue();
                case "closed_lanes" -> closedLanes = readClosedLanes(parser);
                case "restriction" -> restriction = readIntegerList(parser);
                case "speed_limit" -> speedLimit = parser.getIntValue();
                case "incident_indication" -> incidentIndication = readCauseCode(parser);
                case "recommended_path" -> recommendedPath = readDeltaReferencePositionList(parser);
                case "starting_point_speed_limit" -> startingPointSpeedLimit = readDeltaReferencePosition(parser);
                case "traffic_flow_rule" -> trafficFlowRule = parser.getIntValue();
                case "reference_denms" -> referenceDenms = readReferenceDenms(parser);
                default -> parser.skipChildren();
            }
        }
    ...
    private ClosedLanes readClosedLanes(JsonParser parser) throws IOException {
        expect(parser.getCurrentToken(), JsonToken.START_OBJECT);
        Integer hardShoulderStatus = null;
        Integer drivingLaneStatus = null;

        while (parser.nextToken() != JsonToken.END_OBJECT) {
            String field = parser.currentName();
            parser.nextToken();
            switch (field) {
                case "hard_shoulder_status" -> hardShoulderStatus = parser.getIntValue();
                case "driving_lane_status" -> drivingLaneStatus = parser.getIntValue();
                default -> parser.skipChildren();
            }
        }
    ...
    private List<DeltaReferencePosition> readDeltaReferencePositionList(JsonParser parser) throws IOException {
        expect(parser.getCurrentToken(), JsonToken.START_ARRAY);
        List<DeltaReferencePosition> positions = new ArrayList<>();
        while (parser.nextToken() != JsonToken.END_ARRAY) {
            positions.add(readDeltaReferencePosition(parser));
        }
        return positions;
    }

    private List<ActionId> readReferenceDenms(JsonParser parser) throws IOException {
        expect(parser.getCurrentToken(), JsonToken.START_ARRAY);
        List<ActionId> references = new ArrayList<>();
        while (parser.nextToken() != JsonToken.END_ARRAY) {
            references.add(readActionId(parser));
        }
        return references;
    }

    private StationaryVehicle readStationaryVehicle(JsonParser parser) throws IOException {
        expect(parser.getCurrentToken(), JsonToken.START_OBJECT);
        Integer stationarySince = null;
        CauseCode stationaryCause = null;
        CarryingDangerousGoods carryingDangerousGoods = null;
        Integer numberOfOccupants = null;
        VehicleIdentification vehicleIdentification = null;
        Integer energyStorageType = null;

        while (parser.nextToken() != JsonToken.END_OBJECT) {
            String field = parser.currentName();
            parser.nextToken();
            switch (field) {
                case "stationary_since" -> stationarySince = parser.getIntValue();
                case "stationary_cause" -> stationaryCause = readCauseCode(parser);
                case "carrying_dangerous_goods" -> carryingDangerousGoods = readCarryingDangerousGoods(parser);
                case "number_of_occupants" -> numberOfOccupants = parser.getIntValue();
                case "vehicle_identification" -> vehicleIdentification = readVehicleIdentification(parser);
                case "energy_storage_type" -> energyStorageType = parser.getIntValue();
                default -> parser.skipChildren();
            }
        }
    ...
    private CarryingDangerousGoods readCarryingDangerousGoods(JsonParser parser) throws IOException {
        expect(parser.getCurrentToken(), JsonToken.START_OBJECT);
        Integer dangerousGoodsType = null;
        Integer unNumber = null;
        Boolean elevatedTemperature = null;
        Boolean tunnelsRestricted = null;
        Boolean limitedQuantity = null;
        String emergencyActionCode = null;
        ...
        while (parser.nextToken() != JsonToken.END_OBJECT) {
            ...
            parser.nextToken();
            switch (field) {
                case "dangerous_goods_type" -> dangerousGoodsType = parser.getIntValue();
                case "un_number" -> unNumber = parser.getIntValue();
                case "elevated_temperature" -> elevatedTemperature = parser.getBooleanValue();
                case "tunnels_restricted" -> tunnelsRestricted = parser.getBooleanValue();
                case "limited_quantity" -> limitedQuantity = parser.getBooleanValue();
                case "emergency_action_code" -> emergencyActionCode = parser.getValueAsString();
                case "phone_number" -> phoneNumber = parser.getValueAsString();
                case "company_name" -> companyName = parser.getValueAsString();
                default -> parser.skipChildren();
            }
        }
    ...
    private VehicleIdentification readVehicleIdentification(JsonParser parser) throws IOException {
        expect(parser.getCurrentToken(), JsonToken.START_OBJECT);
        String wMINumber = null;
        String vDS = null;
        ...
        while (parser.nextToken() != JsonToken.END_OBJECT) {
            ...
            parser.nextToken();
            switch (field) {
                case "w_m_inumber" -> wMINumber = parser.getValueAsString();
                case "v_d_s" -> vDS = parser.getValueAsString();
                default -> parser.skipChildren();
            }
        }
