...
/**
 * Static validation utility for CAM 2.4.0 envelopes (structured JSON or ASN.1 payloads).
 */
public final class CamValidator240 {
    ...
    private CamValidator240() {
    }

    public static void validateEnvelope(CamEnvelope240 envelope) {
        ...
        requireEquals("version", envelope.version(), "2.4.0");

        if (envelope.linkedStationId() != null) {
            checkRange("linked_station_id", envelope.linkedStationId(), 0, 4294967295L);
        }

        CamMessage240 payload = requireNonNull("message", envelope.message());
