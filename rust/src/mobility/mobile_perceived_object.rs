...
impl MobilePerceivedObject {
    ...
    pub(crate) fn new(
        ...
        let (position, heading) = match &cpm.originating_vehicle_container {
            Some(originating_vehicle_container) => {
                ...
                (
                    ...
                    compute_heading_from_mobile(
                        cartesian_velocity.x_velocity.value,
                        cartesian_velocity.y_velocity.value,
                        originating_vehicle_container.orientation_angle.value as f64 / 100.,
                    ),
                )
            }
            None => {
        ...
        let acceleration = compute_acceleration(&perceived_object);
...
fn compute_acceleration(perceived_object: &PerceivedObject) -> f64 {
    // Placeholder logic for acceleration computation
    perceived_object
        .velocity
        .as_ref()
        .and_then(|v| v.cartesian_velocity.as_ref())
        .map_or(0.0, |v| {
            (v.x_velocity.value.pow(2) + v.y_velocity.value.pow(2)) as f64
        })
}
