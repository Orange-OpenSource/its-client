...
use crate::transport::mqtt::str_topic::StrTopic;
...
impl<const ROUTE_LEVEL: u8> RoutedStrTopic<ROUTE_LEVEL> {
    ...
    pub fn update_topic(&mut self, level: u8, value: &str) {
        if level > 0 {
            let string_topic = self.topic.to_string();
            let parts = string_topic.split('/').collect::<Vec<&str>>();
            let mut new_parts = parts.clone();
            if level as usize <= parts.len() {
                new_parts[level as usize - 1] = value;
                self.topic = StrTopic::from_str(&new_parts.join("/")).unwrap();
            } else {
                warn!("Cannot update topic at to high level {}", level);
            }
        } else {
            warn!("Cannot update topic at level 0")
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn routed_str_topic_not_update_topic_at_level_0() {
        let mut topic = create_routed_str_topic("a/b/c/d");
        topic.update_topic(0, "x");
        assert_eq!(topic.to_string(), "a/b/c/d");
    }

    #[test]
    fn routed_str_topic_update_topic_at_first_level() {
        let mut topic = create_routed_str_topic("a/b/c/d");
        topic.update_topic(1, "x");
        assert_eq!(topic.to_string(), "x/b/c/d");
    }

    #[test]
    fn routed_str_topic_update_topic_at_level_2() {
        let mut topic = create_routed_str_topic("a/b/c/d");
        topic.update_topic(2, "x");
        assert_eq!(topic.to_string(), "a/x/c/d");
    }

    #[test]
    fn routed_str_topic_update_topic_at_level_3() {
        let mut topic = create_routed_str_topic("a/b/c/d");
        topic.update_topic(3, "x");
        assert_eq!(topic.to_string(), "a/b/x/d");
    }

    #[test]
    fn routed_str_topic_update_topic_at_last_level() {
        let mut topic = create_routed_str_topic("a/b/c/d");
        topic.update_topic(4, "x");
        assert_eq!(topic.to_string(), "a/b/c/x");
    }

    #[test]
    fn routed_str_topic_not_update_topic_at_level_too_high() {
        let mut topic = create_routed_str_topic("a/b/c/d");
        topic.update_topic(5, "x");
        assert_eq!(topic.to_string(), "a/b/c/d");
    }
}
