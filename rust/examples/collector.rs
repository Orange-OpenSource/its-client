...
/// Retrieves the exporter configuration from the main configuration.
...
mod tests {
    ...
    fn get_exporter_configuration_defaults() {
        ...
        ini.with_section(Some("mqtt"))
            ...
            .set("port", "1883");
        let config = Configuration::try_from(ini).expect("Failed to create empty configuration");
        let exporter_config = get_exporter_configuration(&config);
    ...
    fn get_exporter_configuration_from_valid_config() {
        ...
        config.set(Some(EXPORTER_SECTION), "stdout", "true");
        config.set(Some(EXPORTER_SECTION), "file", "true");
        config.set(Some(EXPORTER_SECTION), "mqtt", "true");
        let exporter_config = get_exporter_configuration(&config);
        assert!(exporter_config.stdout);
        assert!(exporter_config.file);
        assert!(exporter_config.mqtt);
    ...
    fn get_exporter_configuration_from_invalid_config() {
        ...
        let mut config =
            Configuration::try_from(ini).expect("Failed to create empty configuration");
        config.set(Some(EXPORTER_SECTION), "stdout", "invalid");
        config.set(Some(EXPORTER_SECTION), "file", "invalid");
        config.set(Some(EXPORTER_SECTION), "mqtt", "invalid");
        let exporter_config = get_exporter_configuration(&config);
        assert!(!exporter_config.stdout);
        assert!(!exporter_config.file);
        assert!(!exporter_config.mqtt);
    ...
    fn get_exporter_configuration_with_missing_section() {
        ...
        ini.with_section(Some("mqtt"))
            ...
            .set("client_id", "client-id")
            .set("port", "1883");
        let config = Configuration::try_from(ini).expect("Failed to create configuration");
        let exporter_config = get_exporter_configuration(&config);
        assert!(!exporter_config.stdout);
        assert!(!exporter_config.file);
        assert!(!exporter_config.mqtt);
