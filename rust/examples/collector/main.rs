...
async fn main() {
    ...
    let (mut publish_client, topic_level_update_list) = match &exporter.mqtt {
        Some(configuration) => {
            ...
            // Assumed clone, we copy the prefix
                        ...
                        Some(payload) => {
                                ...
                                && let Some(ref mut current_publish_client) = publish_client
                            {
                                ...
                                for (level, value) in topic_level_update_list {
                                    topic.update_topic(*level, value);
                                }
...
mod tests {
    ...
    fn receiver_configuration_default() {
        let config = ReceiverConfiguration::default();
        assert_eq!(config.topic_list, vec!["#".to_string()]);
    }

    #[test]
    fn receiver_configuration_from_config_with_topics() {
        let mut configuration = Configuration::default();
    ...
    #[test]
    fn receiver_configuration_from_config_empty_list() {
        let mut configuration = Configuration::default();
    ...
    #[test]
    fn receiver_configuration_from_config_missing_field() {
