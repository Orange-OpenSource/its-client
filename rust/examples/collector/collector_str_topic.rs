...
impl CollectorStrTopic {
    ...
    pub fn update_topic(&mut self, level: u8, value: &str) {
        match self {
            CollectorStrTopic::Level1(topic) => topic.update_topic(level, value),
            CollectorStrTopic::Level2(topic) => topic.update_topic(level, value),
            CollectorStrTopic::Level3(topic) => topic.update_topic(level, value),
            CollectorStrTopic::Level4(topic) => topic.update_topic(level, value),
            CollectorStrTopic::Level5(topic) => topic.update_topic(level, value),
            CollectorStrTopic::Default(topic) => topic.update_topic(level, value),
        }
    }
}
